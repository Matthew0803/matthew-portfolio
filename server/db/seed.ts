...
async function seed() {
  ...
  const projectRows = [
    ...
    },
    {
      ...
      description: "A full-stack library management application with Google OAuth authentication, built as a monorepo with a Next.js frontend and Flask REST API backend.\n",
      longDescription: "A full-stack library management system organized as a monorepo with separate frontend and backend deployments. The frontend is built with Next.js and TypeScript, using NextAuth with Google OAuth for authentication. The backend is a Flask REST API secured with JWT, deployed on Railway, while the frontend is deployed on Vercel. The two services communicate across domains with proper CORS configuration.\n",
      technologies: JSON.stringify(["Next.js","TypeScript","Python","Google OAuth","Vercel","Railway"]),
      keyFeatures: JSON.stringify(["Google OAuth authentication via NextAuth","Flask REST API with JWT security","Monorepo architecture with separate frontend and backend","Cross-domain CORS configuration","Frontend deployed on Vercel, backend on Railway"]),
      learnings: JSON.stringify(["Designing a monorepo with independent deployments","Implementing OAuth authentication with NextAuth","Building and securing a REST API with Flask and JWT","Configuring CORS for cross-domain communication","Coordinating Vercel and Railway deployment environments"]),
      imageUrl: "/uploads/project-images/1773020482009-47411003.png",
      videoUrl: null,
      demoUrl: "https://librarry-management-system-web.vercel.app",
      githubUrl: "https://github.com/Matthew0803/librarry-management-system",
      showGithub: true,
      showDemo: true,
      developing: false,
      featured: false,
